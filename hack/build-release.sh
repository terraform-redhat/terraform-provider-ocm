...
function build_cmds {
  ...
  if [ -f "terraform-provider-ocm.exe" ]
  then
    mv "terraform-provider-ocm.exe" "terraform-provider-ocm-${os}-${arch}.exe"
    sha256sum "terraform-provider-ocm-${os}-${arch}.exe" > "terraform-provider-ocm-${os}-${arch}.sha256"
  else
    mv "terraform-provider-ocm" "terraform-provider-ocm-${os}-${arch}"
    sha256sum "terraform-provider-ocm-${os}-${arch}" > "terraform-provider-ocm-${os}-${arch}.sha256"
  fi
}
